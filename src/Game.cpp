      ...
      mPlayer( 50, 50, renderer )
{
  ...
  {
    std::cerr << "Failed to set fullscreen mode: " << SDL_GetError()
              << std::endl;
  }
  ...
  {
    std::cerr << "Failed to load jump sound! SDL_mixer Error: "
              << Mix_GetError() << std::endl;
  }
  ...
  {
    std::cerr << "Failed to load small font! SDL_ttf Error: " << TTF_GetError()
              << std::endl;
  }
...
void Game::loadLevelFromJSON( const std::string &filePath )
{
  ...
  if( json_object_object_get_ex( root, "doors", &doors ) )
  {
    for( int i = 0; i < json_object_array_length( doors ); ++i )
    {
      ...
      float x = json_object_get_double( json_object_object_get( door, "x" ) ) *
        tileSize;
      float y = json_object_get_double( json_object_object_get( door, "y" ) ) *
        tileSize;
      std::string target =
...
void Game::update()
{
  ...
  std::cout << "Player Position: (" << mPlayer.getPosX() << ", "
            << mPlayer.getPosY() << ")\n";
  ...
  float playerCenterX = mPlayer.getX() + mPlayer.getWidth() / 2;
  float playerCenterY = mPlayer.getY() + mPlayer.getHeight() / 2;
  ...
  mCamera.x = static_cast<int>( std::max(
    ...
    std::min( static_cast<float>( mCamera.x ),
              static_cast<float>( LEVEL_WIDTH ) - effectiveScreenWidth ) ) );

  mCamera.y = static_cast<int>( std::max(
    0.0f,
    std::min( static_cast<float>( mCamera.y ),
              static_cast<float>( LEVEL_HEIGHT ) - effectiveScreenHeight ) ) );
...
void Game::render()
{
  ...
  for( auto &platform : mPlatforms )
  {
    if( platform.isVisible(
          mCamera.x, mCamera.y, effectiveScreenWidth, effectiveScreenHeight ) )
    {
      platform.render( mRenderer, mCamera.x, mCamera.y );
      std::cout << "Rendering Platform at (" << platform.getX() << ", "
                << platform.getY() << ")\n";
    }
  }
  ...
  {
    if( solidPlatform.isVisible(
          mCamera.x, mCamera.y, effectiveScreenWidth, effectiveScreenHeight ) )
    {
      solidPlatform.render( mRenderer, mCamera.x, mCamera.y );
      std::cout << "Rendering SolidPlatform at (" << solidPlatform.getX()
                << ", " << solidPlatform.getY() << ")\n";
    }
  }
  ...
  for( auto &crate : mCrates )
  {
          ...
          mCamera.x, mCamera.y, effectiveScreenWidth, effectiveScreenHeight ) )
    {
      ...
      std::cout << "Rendering Crate at (" << crate.getX() << ", "
                << crate.getY() << ")\n";
    }
  }
